	...
	void doFetch(ActionEvent event) {
		...
		try {
			...
			while (table.next()) {
				...
				System.out.println(oid + "   " + contact + "   " + dress + "   " + workers + "   " + mrs + "   " + dos
						+ "   " + amount + "  " + status);
	...
	void doShowDataInExcel(MouseEvent event) throws Exception {
		...
		try {
			...
			for (TableeBean p : list) {
				text = p.getOid() + "," + p.getContact() + "," + p.getDress() + "," + p.getWorker() + ","
						+ p.getMeasurement() + "," + p.getDdate() + "," + p.getAmount() + "," + p.getStatus() + "\n";
				writer.write(text);
			}
			System.out.println("Exported Successfully as Excel format.");
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			writer.flush();
		...
		if (radP.isSelected()) {
			try {
				...
				while (table.next()) {
					...
					int amount = table.getInt("amount");
					int status = table.getInt("status");
					System.out.println(oid + "   " + contact + "   " + dress + "   " + workers + "   " + mrs + "   "
							+ dos + "   " + amount + "  " + status);
	...
	void doShowAll(ActionEvent event) {
		...
		try {
			...
			while (table.next()) {
				...
				System.out.println(oid + "   " + contact + "   " + dress + "   " + worker + "   " + mrs + "   " + dos
						+ "   " + amount + "  " + status);
				TableeBean obj = new TableeBean(oid, contact, dress, worker, mrs, dos.toString(), amount, status);
				list.add(obj);
			}
