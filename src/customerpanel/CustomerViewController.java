...
public class CustomerViewController {
	...
	void doDetails(ActionEvent event) {
		try {
			...
			if(comboCn.getEditor().getText()=="")
			{
				showMsg("Please Fill Mobile Number.");
				return;
			}
			...
			while (table.next()) {
				...
				Date date = table.getDate(8);// 8th column
				String ref = table.getString("ref");

				System.out.println(name + "   " + picpath + "   " + address + "   " + city + "   " + email + "   " + str
						+ "   " + date + "   " + ref);
	...
	void doModify(ActionEvent event) {
		try {
			pst = con.prepareStatement(
					"update customers set name=?,picpath=?,address=?,city=?,email=?,gender=?,ref=? where contact=?");
